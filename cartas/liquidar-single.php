...
if(isset($_POST['btn-signup'])) {
	...
	if (isset($_POST['m_ids']) and !empty($_POST['m_ids'])) { // Finalizo Movimientos Seleccionados
		$ids = explode(" ", $_POST['m_ids']);
		foreach ($ids as $id_act) {
			$query = $mysqli->prepare("UPDATE trans_cc SET Fin = 1, CP_Id = ? WHERE Id = ?" );
			$query->bind_param('ii', $CpId, $id_act);
			if($query->execute()) {
					$query->close();
			} else {
				$error = true;
	...
	$m_fin = 1;
	$m_fecha = date('Y-m-d');
	$m_nota = 'Carta de Porte #' . $m_ncp . ( !empty($m_dta) ? ' | ' . $m_dta : ''); 
	$query = $mysqli->prepare("INSERT INTO trans_cc(Fecha, Tipo, CP_Id, Tr_Id, Importe, Nota, Fin) VALUES (?,?,?,?,?,?,?)");
	$query->bind_param('siiidsi', $m_fecha, $m_tipo, $CpId, $TrId, $m_importe, $m_nota, $m_fin);
	if($query->execute()) {
			$query->close();
	} else {
			$error = true;
	...
	$sql = "SELECT Importe, Nota FROM trans_cc WHERE CP_Id = " . $CpId . " AND Tipo = 0";
	if (!$recset = $mysqli->query($sql)) {
			echo "<h2>Error en la Consulta SQL | Importe CC.</h2>";
			exit;
	}	else {
		$totalcp = $recset->fetch_assoc();
	}
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>Liquidar Carta de Porte <small>#<?php echo $carta['Numero'];?></small></h1>
  </section>
    ...
    <div class="col-md-12">
      ...
      <div class="box box-info">
          <div class="box-header with-border">
            ...
            <span class="text-muted pull-right"><small>Seleccione los Movimientos a <strong>descontar</strong> del Total de la Carta de Porte...</small></span>
          </div>
          <div class="box-body">
              ...
              ?>
							...
							<script type="text/javascript">
								var Movimientos = [];
								var totalcp = <?php echo $totalcp['Importe'];?>;
							</script>
							
              <div class="row">
                <div class="col-sm-12">
                	<div class="callout callout-info">
										<h4 class="text-center"><?php echo $totalcp['Nota'];?></h4>
                  </div>
                </div>
              </div>							
              ...
              <div class="row">
                <div class="col-sm-12">
                	<div class="callout callout-warning">
                  	<span id="">Total Carta de Porte:</span><span class="pull-right">$ <?php echo number_format($totalcp['Importe'],2,',','.');?></span>
                  </div>
                </div>
              </div>							
                  ...
                  <div class="form-group">
                    <div class="input-group">
                      ...
                      <input type="text" class="form-control" value="" placeholder="Ingrese aqui informaci&oacute;n adicional como # de comprobante..." name="m_datadc" maxlength="110"/>
							...
							<input type="hidden" name="m_ids" id="m_ids" value="" />
							<input type="hidden" name="m_ncp" id="m_ncp" value="<?php echo $carta['Numero'];?>" />
              ...
              <div class="row">
                ...
                <div class="form-group">
                  ...
                  <button type="submit" class="btn btn-primary pull-right" name="btn-signup" id="btn-signup"><i class="fa fa-download"></i> &nbsp; Liquidar Carta de Porte</button> 
                </div>
                </div>
              </div>
